...
int main(int argc,char**argv){
    ...
    auto process=[&](AVFrame* f){
        ...
        tMot+=ms(m0,clk::now());
        bool active=hasPrev&&vr[0]>=minCells;hasPrev=true;
        // ---- preprocess (only on motion — the gated path) ----
        if(active){++motion;
            ...
            // CHW is now in host-visible chwMap = the handoff payload to NVIDIA
            volatile float sink=((float*)chwMap)[0];(void)sink;
            tPre+=ms(p0,clk::now());
    ...
    printf("\n=== mixed-mode iGPU front-half (decode+motion+preprocess) ===\n");
    printf("frames=%d  motion(gated)=%d (%.0f%%)\n",frames,motion,frames?100.0*motion/frames:0);
    printf("decode (VAAPI/VCN) : %.2f ms/frame\n",frames?tDec/frames:0);
    printf("motion (Vulkan)    : %.2f ms/frame\n",frames?tMot/frames:0);
    printf("preprocess (Vulkan): %.2f ms/gated-frame\n",motion?tPre/motion:0);
    printf("iGPU front-half throughput: %.1f fps (%.0f ms total)\n",frames?frames*1000.0/wall:0,wall);
    printf("handoff payload to NVIDIA  : %.2f MB/gated-frame (CHW %d*%d*3 f32)\n",3.0*NET*NET*4/1e6,NET,NET);
    return 0;
}
