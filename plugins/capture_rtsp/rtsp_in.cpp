...
struct RtspContext {
    ...
    void publish_frame(const zm_frame_hdr_t* hdr, const void* data, size_t size) {
        if (host_api && host_api->on_frame) {
            // Callback expects the header followed by packet data in a contiguous buffer
            if (hdr->bytes > 0 && data) {
                // Allocate buffer for header + data
                uint8_t* buffer = new uint8_t[sizeof(zm_frame_hdr_t) + size];
                // Copy header
                memcpy(buffer, hdr, sizeof(zm_frame_hdr_t));
                // Copy packet data
                memcpy(buffer + sizeof(zm_frame_hdr_t), data, size);
                // Send to host
                host_api->on_frame(host_ctx, buffer, sizeof(zm_frame_hdr_t) + size);
                // Clean up
                delete[] buffer;
            } else {
                // Only header (for zero-sized packets or GPU surfaces)
                host_api->on_frame(host_ctx, hdr, sizeof(zm_frame_hdr_t));
            }
        }
    }
};
